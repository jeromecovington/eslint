...
* `"preferType"` enforces consistent type strings specified by an object whose properties mean **disallow** the key and **allow** the value (for example, `"object": "Object"` means disallow `object` and allow `Object`)
* `"requireReturn": true` (default) requires a return tag **even if** the function or method does not have a `return` statement (but it does not apply to constructor functions)
* `"requireReturn": false` requires a return tag **if and only if** the function or method has a `return` statement
* `"requireReturnType": false` allows missing type in return tags
* `"matchDescription"` specifies (as a string) a regular expression to match the description in each JSDoc comment (but it does not apply to descriptions in parameter or return tags)
